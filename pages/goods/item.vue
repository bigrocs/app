  ...
  <view class="content">
    <view v-if="Object.keys(goods).length > 0" class="jingdong">
      <view class="left">
        ...
        <view
          class="type"
          v-if="
            (this.roles.indexOf('root') > -1 ||
              this.roles.indexOf('manager') > -1 ||
              this.roles.indexOf('director') > -1 ||
              this.roles.indexOf('finance') > -1 ||
              this.roles.indexOf('store_keeper') > -1) &&
            goods.buyPrice
          "
          >进价: ￥ {{ goods.buyPrice }}</view
        >
        ...
        <view class="tips">
          <view class="explain">
            <view
              >更新:
              {{
                goods.updatedAt | parseTime("{y}-{m}-{d} {h}:{i}:{s} 星期{a}")
              }}</view
            >
          </view>
        </view>
    ...
    <view
      v-if="
        ...
        goods.hasOwnProperty('stock')
      "
      class="supplier"
    >
      <view>供应商库存: </view>
      <view
        v-for="(item, index) in goods.stock.supplier"
        :key="index"
        class="item"
      >
        <view class="bottom">
        ...
        await this.chackLocation()
          .then((res) => {
            ...
            if (
              res.latitude > 37.141 &&
              res.latitude < 37.143 &&
              res.longitude > 118.131 &&
              res.longitude < 118.133
            ) {
              this.branch = "boxing";
              this.branchName = "博兴";
            ...
            if (
              res.latitude > 37.253 &&
              res.latitude < 37.258 &&
              res.longitude > 118.21 &&
              res.longitude < 118.218
            ) {
